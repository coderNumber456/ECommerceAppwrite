...
function Header() {
    ...
    const authFlag = useSelector((state)=>state.auth.flag)
    const dispatch = useDispatch()
    


    console.log("status1",authStatus)
    console.log("flag1" , authFlag)
    
  
    const [display,setDisplay] = useState(false)
    const [authFlagStatus,setAuthFlagStatus] = useState(false)
    ...
    useEffect(()=>{
        const getUser = async()=>{
          try {
            ...
            if(userData.email!==""){
             setAuthFlagStatus(true)
             dispatch(sellFlag(true))
            }
          } catch (error) {
            console.log(error)
            dispatch(sellFlag(false))
          }
        };getUser()
    },[authStatus,authFlag])

    console.log(authData, " authData")
    console.log(authFlagStatus , " falg status")
